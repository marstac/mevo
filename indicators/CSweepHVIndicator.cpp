...
int CSweepHVIndicator::avl_check_balance(avl_node_t *avlnode) {
#ifdef AVL_DEPTH
    int d;
...
int CSweepHVIndicator::avl_search_closest(const avl_tree_t *avltree, const void *item, avl_node_t **avlnode) {
    avl_node_t *node;
    int c;
...
avl_tree_t * CSweepHVIndicator::avl_init_tree(avl_tree_t *rc) {
    if(rc) {
        rc->head = NULL;
        rc->tail = NULL;
        rc->top = NULL;
...
avl_tree_t * CSweepHVIndicator::avl_alloc_tree() {
    return avl_init_tree(new avl_tree_t);
}

void CSweepHVIndicator::avl_clear_tree(avl_tree_t *avltree) {
    avltree->top = avltree->head = avltree->tail = NULL;
}

void CSweepHVIndicator::avl_clear_node(avl_node_t *newnode) {
...
avl_node_t * CSweepHVIndicator::avl_insert_top(avl_tree_t *avltree, avl_node_t *newnode) {
...
avl_node_t *CSweepHVIndicator::avl_insert_before(avl_tree_t *avltree, avl_node_t *node, avl_node_t *newnode) {
/*	if(!node)
        return avltree->tail
...
avl_node_t * CSweepHVIndicator::avl_insert_after(avl_tree_t *avltree, avl_node_t *node, avl_node_t *newnode) {
/*	if(!node)
        return avltree->head
 ...
 */
void CSweepHVIndicator::avl_unlink_node(avl_tree_t *avltree, avl_node_t *avlnode) {
    avl_node_t *parent;
 ...
 */
void CSweepHVIndicator::avl_rebalance(avl_tree_t *avltree, avl_node_t *avlnode) {
    avl_node_t *child;
    avl_node_t *gchild;
    avl_node_t *parent;
    avl_node_t **superparent;
...
{
    const double x1 = *((*(const dlnode **)p1)->x);
    const double x2 = *((*(const dlnode **)p2)->x);

    return (x1 < x2) ? -1 : (x1 > x2) ? 1 : 0;
}

 int CSweepHVIndicator::compare_tree_asc(const void *p1, const void *p2)
{
...
dlnode * CSweepHVIndicator::setup_cdllist(double *data, int d, int n)
{
    dlnode *head;
    dlnode **scratch;
    int i, j;
    ...
    head->next = new dlnode*[d * (n+1)];// malloc(  * sizeof());
    head->prev = new dlnode*[d * (n+1)];// malloc(  * sizeof(dlnode_t*));
    head->tnode = new avl_node_t[n+1];//malloc ((n+1) * sizeof());

#if VARIANT >= 2
    head->area = new double [d * (n+1)]; //malloc(d * (n+1) * sizeof(double));
#endif
#if VARIANT >= 3
    head->vol = new double [d * (n+1)];//malloc(d * (n+1) * sizeof(double));
#endif
    ...
    scratch = new dlnode*[n];// malloc(n * sizeof());
    for (i = 0; i < n; i++)
        scratch[i] = head + i + 1;
    ...
    for (j = d-1; j >= 0; j--) {
        for (i = 0; i < n; i++)
            scratch[i]->x--;
        qsort(scratch, n, sizeof(dlnode*), compare_node);
...
void CSweepHVIndicator::free_cdllist(dlnode * head)
{
...
void CSweepHVIndicator::ndelete (dlnode *nodep, int dim, double * bound)
{
    int i;
...
#if VARIANT >= 2
void CSweepHVIndicator::delete_dom (dlnode *nodep, int dim)
{
    int i;
...
void CSweepHVIndicator::reinsert (dlnode *nodep, int dim, double * bound)
{
    int i;
...
#if VARIANT >= 2
void CSweepHVIndicator::reinsert_dom (dlnode *nodep, int dim)
{
    int i;
    for (i = stop_dimension; i < dim; i++) {
        dlnode *p = nodep->prev[i];
        p->next[i] = nodep;
        nodep->next[i]->prev[i] = nodep;
...
double CSweepHVIndicator::hv_recursive(dlnode *list, int dim, int c, const double * ref,
             double * bound)
{
    /* ------------------------------------------------------
...
*/
void CSweepHVIndicator::filter_delete_node(dlnode *node, int d)
{
    int i;
...
*/
int CSweepHVIndicator::filter(dlnode *list, int d, int n, const double *ref)
{
    int i, j;

    /* fprintf (stderr, "%d points initially\n", n); */
    for (i = 0; i < d; i++) {
        dlnode *aux = list->prev[i];
        int np = n;
        for (j = 0; j < np; j++) {
            if (aux->x[i] < ref[i])
                break;
...
double CSweepHVIndicator::fpli_hv(double *data, int d, int n, const double *ref)
{
    dlnode *list;
    double hyperv;
    double * bound = NULL;
...
#if VARIANT >= 3
    bound = new double [d];//;malloc (d * sizeof(double));
...
CSweepHVIndicator::CSweepHVIndicator()
{
#if VARIANT < 4
stop_dimension = 1; /* default: stop on dimension 2 */
#else
stop_dimension = 2; /* default: stop on dimension 3 */
#endif
}
...
double CSweepHVIndicator::calculateHypervolume(std::deque<std::deque<double> > &_front)
{
    double *data = new double[_front.size()*_front[0].size()];
    int cnt=0;
    for (int i=0; i< _front.size(); i++) {
         for (int j=0; j<_front[i].size(); j++) {
              data[cnt] = _front[i][j];cnt++;
         }
    }
    double *ref = new double[FRefPoint.size()];

    for (int i=0; i< _front[0].size(); i++){ref[i]=FRefPoint[i];}

    double hv = fpli_hv(data,_front[0].size(), _front.size(), ref);
...
const char *CSweepHVIndicator::getName()
{
    return "Sweep hypervolume indicator";
}
}
